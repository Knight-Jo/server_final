	...
	if (rc != SQLITE_OK) {
		...
		if (tablenum == 1) {
			...
			while (sqlite3_step(pStmt) == SQLITE_ROW) {
				...
				u.UName = std::string(reinterpret_cast<const char*>(sqlite3_column_text(pStmt, 1)));
				u.Password = std::string(reinterpret_cast<const char*>(sqlite3_column_text(pStmt, 2)));
				u.Potrait = std::string(reinterpret_cast<const char*>(sqlite3_column_text(pStmt, 3)));
				u.LogStatus = sqlite3_column_int(pStmt, 4);
				u.Socket = sqlite3_column_int(pStmt, 5);
				u.OffLineTime = std::string(reinterpret_cast<const char*>(sqlite3_column_text(pStmt, 6)));
				User2[i] = u;
				i++;
			}
			User1 = User2;
			...
			Cluster1.clear();
			vector<Cluster> Cluster2(row);
			Cluster c;
			int i = 0;
			while (sqlite3_step(pStmt) == SQLITE_ROW) {
				c.CNum = sqlite3_column_int(pStmt, 0);
				c.CName = std::string(reinterpret_cast<const char*>(sqlite3_column_text(pStmt, 1)));
				c.COwner = sqlite3_column_int(pStmt, 2);
				c.CMember = std::string(reinterpret_cast<const char*>(sqlite3_column_text(pStmt, 3)));
				c.MsgRecord = std::string(reinterpret_cast<const char*>(sqlite3_column_text(pStmt, 4)));
				Cluster2[i] = c;
				i++;
			}
			Cluster1 = Cluster2;
